...
class DriversAgeExport implements  FromQuery,
ShouldAutoSize,
WithMapping,
WithHeadings,
WithEvents,
    ...
    */
    ...
    public function map($driver): array{
            ...
            if ((preg_match($pattern, $driver->employee->start_date)) ){
                $start_date = Carbon::parse($driver->employee->start_date);
                $yearsDifference = $start_date->diffInYears($today);
            }else {
                $yearsDifference = "";
            }
            if ((preg_match($pattern, $driver->employee->end_date)) ){
                $end_date = Carbon::parse($driver->employee->end_date);
            ...
            return   [
                $driver->employee->name ." ". $driver->employee->surname,
                $driver->employee->start_date,
                $yearsDifference ? $yearsDifference." Year(s)" : "",
                $driver->employee->end_date,
