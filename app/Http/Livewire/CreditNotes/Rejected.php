...
class Rejected extends Component
{
    ...
    public function mount(){
        ...
        if (isset( $period)) {
            if ($period != "all") {
                $this->credit_notes = CreditNote::where('authorization', 'rejected')->whereYear('created_at',$period)->latest()->get();
            }else {
                $this->credit_notes = CreditNote::where('authorization', 'rejected')->latest()->get();
            }
        }
    ...
    public function authorize($id){
      ...
      try{
        ...
        if ($this->authorize == "approved") {
            ...
            $this->dispatchBrowserEvent('alert',[
                'type'=>'success',
                'message'=>"Credit Note Rejected Already"
            ]);
    ...
    {
        $this->credit_notes = CreditNote::where('authorization', 'rejected')->latest()->get();
        return view('livewire.credit-notes.rejected',[
