...
class Index extends Component
{
    ...
    public function mount($category){
            ...
            if (in_array('Admin', $role_names) || in_array('Super Admin', $role_names)) {
                $this->purchases = Purchase::whereDate('created_at', \Carbon\Carbon::today())
                ->where('department',$this->department)->latest()->take(5)->get();
            } else {
    ...
    {
        if (!is_null($id) ) {
            $this->products = Product::where('category_value_id', $id)->orderBy('name','asc')->get();
        }
    }
