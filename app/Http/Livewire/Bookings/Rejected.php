    ...
    public function mount(){
      ...
      public function update(){
        ...
        if ($this->authorize == "approved") {
            ...
            $this->dispatchBrowserEvent('alert',[
                'type'=>'success',
                'message'=>"Booking Rejected Already"
                 ]);
    ...
    public function render()
    {
        ...
        if (in_array('Admin', $role_names) || in_array('Super Admin', $role_names)) {
            if (isset($this->from) && isset($this->to)) {
                if (isset($this->search)) {
                    return view('livewire.bookings.rejected',[
                        'bookings' => Booking::query()->with('ticket','inspection','horse','trailer','vehicle')->where('authorization','rejected')->whereBetween('created_at',[$this->from, $this->to] )
                    ...
                    return view('livewire.bookings.rejected',[
                        'bookings' => Booking::query()->with('ticket','inspection','horse','trailer','vehicle')->where('authorization','rejected')->whereBetween('created_at',[$this->from, $this->to] )->orderBy('booking_number','desc')->paginate(10),
            ...
            if (isset($this->from) && isset($this->to)) {
                if (isset($this->search)) {
                    return view('livewire.bookings.rejected',[
                        'bookings' => Booking::query()->with('ticket','inspection','horse','trailer','vehicle')->where('authorization','rejected')->whereBetween('created_at',[$this->from, $this->to] )->where('user_id',Auth::user()->id)
                        ->where('booking_number','like', '%'.$this->search.'%')
                        ->orWhereHas('horse', function ($query) {
                            return $query->where('registration_number', 'like', '%'.$this->search.'%');
                        })
                    ...
                    return view('livewire.bookings.rejected',[
                        'bookings' => Booking::query()->with('ticket','inspection','horse','trailer','vehicle')->where('authorization','rejected')->whereBetween('created_at',[$this->from, $this->to] )->where('user_id',Auth::user()->id)->orderBy('booking_number','desc')->paginate(10),
